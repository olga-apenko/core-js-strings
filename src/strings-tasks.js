...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str.charAt(i) === str.charAt(i).toUpperCase())
      stringInvert += str.charAt(i).toLowerCase(char);
    else stringInvert += str.charAt(i).toUpperCase(char);
  }
  return stringInvert;
}
...
function extractNameFromTemplate(value) {
  ...
  if (value.includes(greeting))
    if (value.includes('!')) {
      return value.slice(greeting.length, value.lastIndexOf('!'));
    }
  return value;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (c) => abcRor13[abcNorm.indexOf(c)]);
}
